...
class Sliced_Metaboxes {
	...
	public function main_section() {
		...
		$default_payment_method = '';
                      ...
                      $keys = array_keys($payment_methods);
                      $default_payment_method = $keys[1]; // Set the first method (after the empty key)
                        }
		$payments->add_group_field( $payments_group_id, array(
	...
	public function quote_side() {
		...
		$info->add_field( array(
			'name' => '</label><label>
			<button type="button" id="autofill-button" class="button button-primary">AUTOFILL</button>', // The AUTOFILL button
			'desc' => '',
			'id'   => 'autofill_button',
			'type' => 'html', // Use HTML to render the button
		) );
		
		$info->add_field( array(
			'name'       => __( 'Client', 'sliced-invoices' ) .
			 '</label><label><a href="#TB_inline?width=960&height=650&inlineId=sliced-ajax-update-client" title="Edit Client" class="thickbox button button-small">' .
			  __( 'Edit Client', 'sliced-invoices' ) . 
			 '</a> <a href="#TB_inline?width=960&height=700&inlineId=add-ajax-user" title="Add New Client" class="thickbox button button-small sliced-add-client-button">' .
			  __( 'Add New Client', 'sliced-invoices' ) . '</a>',
			'desc'       => '',
			'id'         => $prefix . 'client',
			'type'       => 'select',
