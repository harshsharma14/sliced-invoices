...
class Sliced_Invoice {
	...
	public function add_autofill_button() {
        ...
        jQuery(document).ready(function($) {
            $('#publish').after('<button id="autofill" class="button">AUTOFILL</button>');
            $('#autofill').click(function(e) {
                e.preventDefault();
                let invoiceId = $('#post_ID').val();
                let ajaxurl = "<?php echo admin_url('admin-ajax.php'); ?>";

                $.post(ajaxurl, {
                    action: 'sliced_autofill',
                    invoice_id: invoiceId
                }, function(response) {
                    // Populate fields with response data
                    $('#payment_date').val(response.date); // For today's date
                    $('#sliced_total').val(response.amount); // For amount
                    $('#_sliced_payment_0_method').val('Bank'); // Set payment method to Bank (make sure this ID matches the actual field)
                    $('#_sliced_invoice_number').val(response.invoice_number); // Payment ID as invoice number
                    $('#_sliced_payment_0_status').val('completed'); // Set status to Completed
                }, 'json');
            });
        });
        </script>
    ...
    public function handle_autofill() {
        ...
        // Get total due amount from line items
        $line_items = get_post_meta($invoice_id, self::$meta_key['items'], true);
        $total_due = array_sum(array_column($line_items, 'amount')); // Assuming line items are stored in an array
        
        // Prepare response data
        $response = array(
            'date' => current_time('Y-m-d'), // Today's date
            'amount' => $total_due,
            'invoice_number' => self::get_number($invoice_id), // Get the invoice number
        );
